...
<template>
  <div class="relative flex h-dvh flex-col items-center justify-center overflow-hidden px-6">
    ...
    <h1 class="relative m-0 mt-10 text-center text-3xl font-semibold tracking-tight sm:text-4xl">
      {{ listening ? "Listening…" : "Play something" }}
    </h1>
    ...
    <p v-else class="relative m-0 mt-3 max-w-sm text-center text-sm leading-relaxed text-mist-400">
      ...
      <template v-else>Tap to listen. Fingerprinting happens here &mdash; only integers are sent.</template>
    </p>
