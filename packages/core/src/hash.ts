import { FAN_OUT, FREQ_FUZZ, TARGET_ZONE_MAX, TARGET_ZONE_MIN } from "./constants";
import type { Peak } from "./peaks";

export interface Fingerprint {
  hash: number;
  frame: number;
}

export interface HashParts {
  anchor: number;
  target: number;
  dt: number;
}

// 22 bits: staying under 31 keeps JS signed bitwise ops from producing a negative hash.
export function packHash(anchorBin: number, targetBin: number, dt: number): number {
  return ((anchorBin >> FREQ_FUZZ) << 14) | ((targetBin >> FREQ_FUZZ) << 6) | dt;
}

export function unpackHash(hash: number): HashParts {
  return { anchor: (hash >> 14) & 0xff, target: (hash >> 6) & 0xff, dt: hash & 0x3f };
}

export function fingerprint(peaks: readonly Peak[]): Fingerprint[] {
  const out: Fingerprint[] = [];

  for (let i = 0; i < peaks.length; i++) {
    const anchor = peaks[i]!;
    let paired = 0;

    for (let j = i + 1; j < peaks.length && paired < FAN_OUT; j++) {
      const target = peaks[j]!;
      const dt = target.frame - anchor.frame;
      if (dt < TARGET_ZONE_MIN) continue;
      if (dt > TARGET_ZONE_MAX) break;

      out.push({ hash: packHash(anchor.bin, target.bin, dt), frame: anchor.frame });
      paired++;
    }
  }

  return out;
}
