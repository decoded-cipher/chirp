...
rare AS (
  SELECT q.hash, q.qframe
  FROM q LEFT JOIN hash_stats hs ON hs.hash = q.hash
  WHERE hs.hash IS NULL
),
tally AS (
  SELECT
    f.song_id,
    CAST(FLOOR((f.anchor_frame - rare.qframe) / ${OFFSET_BUCKET}.0) AS INTEGER) AS offset_bucket,
    COUNT(*) AS votes
  FROM rare
  JOIN fingerprints f ON f.hash = rare.hash
  GROUP BY f.song_id, offset_bucket
  HAVING offset_bucket >= 0
)
SELECT song_id, MAX(votes) AS votes
FROM tally
GROUP BY song_id
ORDER BY votes DESC
LIMIT ?2`;
